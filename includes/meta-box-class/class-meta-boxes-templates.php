  ...
  public function show_field_repeater( $field, $meta ) {
    ...
    echo '<script>
        jQuery(document).ready(function() {
          var '.$counter.' = '.$c.';
          jQuery('.$field['id'].').on(\'click\',"#add-'.$field['id'].'", function() {
            '.$counter.' = '.$counter.' + 1;
            jQuery(this).before(\''.$js_code.'\');            
            update_repeater_fields();
              });
              jQuery('.$field['id'].').on(\'click\',"#remove-'.$field['id'].'", function() { 
...
.at-inline .at-text{width: 20%;}
.at-inline .at-textarea{width: 80%; height: 75px;}
